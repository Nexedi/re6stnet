...
'''
import logging,argparse,time,errno,socket,os
    ...
    def run(self):
        ...
        with csv_lock:
            if len(no_responses)>0:
                my_csv.write("%r,%d,%d,%s\n"%(time.time(),seq,len(no_responses),' '.join(no_responses)))
                my_csv.flush()
            else :
                os.utime(csv_path,(time.time(),time.time())) #update modification/access time of csv
...
parser.add_argument("n", type=str, help="my machine name (m1,m2...)")
