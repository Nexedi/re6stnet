...
class Re6stRegistry(object):
    ...
    def run(self):
        ...
        logging.info("run registry {} at ns: {} with cmd: {}".format(self.name, self.node.pid," ".join(cmd)))
...
class Re6stNode(object):
    ...
    def create_node(self):
        ...
        logging.info("create dir of node {}".format(self.name))
        cmd = "{script} --registry {registry_url} --email {email}"
        cmd = cmd.format(script=RE6ST_CONF, registry_url=self.registry.url,
                         email=self.email).split()
        p = self.node.Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE, cwd=str(self.path))
        # read token
    ...
    def run(self, *args):
        ...
        cmd += args
        logging.info("run node {} at ns: {} with cmd: {}".format(self.name, self.node.pid," ".join(cmd)))
        self.proc = self.node.Popen(cmd, stdout=PIPE, stderr=PIPE)
