...
class RegistryServer:
    ...
    def __init__(self, config):
        ...
        logging.info("Network: %s/%u", utils.ipFromBin(self.network),
                                       len(self.network))
        attrs = self.cert.ca.subject.get_attributes_for_oid(
            NameOID.EMAIL_ADDRESS)
        self.email = attrs[0].value if attrs else None
    ...
    def iterCert(self) -> Iterator[tuple[cx509.Certificate, str, str]]:
        for prefix, email, cert in self.db.execute(
            ...
            cert = cert.encode()
            try:
                yield x509.load_pem_x509_certificate(cert), prefix, email
            except ValueError:
                pass
    ...
    def requestCertificate(self, token, req, location='', ip=''):
        ...
        with self.lock:
            with self.db:
                ...
                return self.createCertificate(prefix, subject, csr.public_key())

    def getSubjectSerial(self):
        # Smallest unique number, for IPv4 support.
        serials = []
        for x in self.iterCert():
            attrs = x[0].subject.get_attributes_for_oid(NameOID.SERIAL_NUMBER)
            if attrs:
    ...
    def createCertificate(self, client_prefix, subject, pubkey, not_after=None):
        ...
        name_attrs = [a for a in subject if a.oid != NameOID.COMMON_NAME]
        name_attrs.append(cx509.NameAttribute(NameOID.COMMON_NAME, cn_value))
        if not_after is None:
            not_after = now + timedelta(seconds=self.cert_duration)
        ...
        cert = cx509.CertificateBuilder(
            issuer_name=self.cert.ca.subject,
            subject_name=cx509.Name(name_attrs),
            public_key=pubkey,
            serial_number=serial,
            not_valid_before=now,
            not_valid_after=not_after,
        ).sign(self.cert.key, hashes.SHA512(), backend=x509.backend)
