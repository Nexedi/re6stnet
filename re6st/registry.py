...
class RegistryServer(object):
    ...
    def handle_request(self, request, method, kw):
        with self.lock:
            self.headers_client = request.headers
    ...
    def getBootstrapPeer(self, cn):
        with self.peers_lock:
            ...
            if peer == cn:
                ...
                peer = self.prefix
        with self.lock:
            forward = self.headers_client.get("X-Forwarded-For")
            addr = forward if forward else self.headers_client.get("host")
            logging.info("Got contact from %s (%s) with user-agent %s",
                utils.ipFromBin(x509.networkFromCa(self.cert.ca) + cn),
                addr, self.headers_client.get('user-agent'))
...
class RegistryClient(object):
    ...
    _hmac = None
    user_agent = platform.platform() +"_re6stnet/" + version.version
