#!/usr/bin/python2
import atexit, copy, errno, logging, os, shutil, signal
...
def main():
    ...
    server_tunnels = {}
    forwarder = None
    phy_iface_list = copy.copy(config.iface_list)
    if config.client:
    ...
    try:
        ...
        if config.multicast:
            plib.pimdm(phy_iface_list, config.run)
            cleanup.append(lambda: subprocess.call(['pim-dm', '-stop']))
